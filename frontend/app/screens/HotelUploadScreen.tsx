...
import { Upload, FileText, Check, ArrowLeft, ArrowRight, X, Building } from 'lucide-react';
import { useApp } from '../../context/AppContext';

export default function HotelUploadScreen() {
  const { setCurrentStep, tripData, setHotelConferenceReceipts } = useApp();
  const [dragActive, setDragActive] = useState(false);
  const [uploadedFiles, setUploadedFiles] = useState<File[]>(tripData.hotelConferenceReceipts);
  ...
  const handleContinue = () => {
    setHotelConferenceReceipts(uploadedFiles);
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-20">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#3B82F6]/10 rounded-full text-[#3B82F6] text-sm font-medium mb-4">
            <span className="w-5 h-5 bg-[#3B82F6] rounded-full text-white text-xs flex items-center justify-center">3</span>
            Step 3 of 3
          </div>
          <h1 className="text-3xl font-bold text-[#1E293B] mb-3">
            Upload Hotel & Conference Receipts
          </h1>
          <p className="text-[#64748B] max-w-md mx-auto">
            Add your hotel invoices, conference fees, and other accommodation receipts
          </p>
        </motion.div>
        ...
        >
          ...
          >
            ...
            >
              ...
              `}>
                <Building className={`w-7 h-7 ${dragActive ? 'text-white' : 'text-[#64748B]'}`} />
              </div>
        ...
        >
          <button
            ...
            className="flex items-center gap-2 px-8 py-4 rounded-full font-medium
              ...
              bg-[#10B981] text-white hover:bg-[#059669] hover:shadow-xl hover:shadow-[#10B981]/25"
          >
